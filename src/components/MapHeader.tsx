      ...
      <StyledHeader>
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          ...
          <Input
            ...
            onPressEnter={async (values) => {
              const res = await companyGetTypeOrders('banana');
              markersDispatch({type: COMPANY_GET_ORDERS, data: res})
              console.log("res", res);
            }}
          />
            <Button
        ...
        <div style={{ lineHeight: "32px", color: "white" }}>{userInfo?.name}</div>
      ...
      >
        {histories.length > 0 ? (
          <List size="large">
            ...
            />
            {histories.map((item) => (
              <div key={item.time} style={{ display: "flex", alignItems: "center" }}>
                <List.Item>
                  <div>{item.time}</div>
